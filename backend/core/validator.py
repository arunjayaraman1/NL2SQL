...
INTENT_PATTERNS = {
    ...
    "joining": {
        ...
        "requires": ["JOIN"],
        "hint": "- 'with X', 'including X' may require JOIN between tables",
    },
    "comparison": {
        "keywords": [
            "more than",
    ...
    "exclusion": {
        ...
        "hint": "- 'without X', 'no X', 'never' may require LEFT JOIN + IS NULL or NOT IN subquery",
    },
    "null_handling": {
        "keywords": ["null", "empty", "missing", "no phone", "no email", "no record"],
    ...
    "subquery": {
        ...
        "requires": ["Subquery"],
        "hint": "- 'more than average', 'exceeds' requires subquery: (SELECT AVG(column) FROM table)",
    },
    "case_expression": {
        "keywords": ["label", "categorized", "classified", "status", "type", "kind"],
        "requires": ["CASE expression"],
        "hint": "- Use CASE WHEN for categorization/labeling",
    },
    "distinct": {
        "keywords": ["distinct", "unique", "different", "without duplicates"],
        "requires": ["DISTINCT"],
